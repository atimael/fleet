...
| `planning` | Turns a feature into a durable OKF plan: task docs with dependencies (`dependent` status), tests, and acceptance criteria, executable without the planning conversation. Run on gpt-5.6-sol high. | auto — ask for an implementation plan |
| `execute-plan` | The loop side of `planning`: a secretary agent (gpt-5.6-luna medium) picks up ready tasks in dependency order, hands each to a Codex worker (gpt-5.6-sol low), verifies with real test runs, keeps task logs, and commits per task. | auto — when `.tasks/okf/` artifacts exist |
