...
| `init-project` | Scaffolds a new project of any stack (Swift, iOS, web, …) into loop-ready state: canonical generator, passing test harness, verified commands in CLAUDE.md, `.tasks/` ignored, initial commit. | auto — say "init project" / `/init-project` |
| `planning` | Turns a feature into a durable OKF plan: task docs with dependencies (`dependent` status) and WHEN/THEN acceptance rows, executable without the planning conversation. Plans can be partial (unsettled work sits under `Not yet specified` until a re-plan). Declines to plan single bounded tasks. Run on gpt-5.6-sol high. | auto — ask for an implementation plan |
| `execute-plan` | The loop side of `planning`: a secretary agent (gpt-5.6-luna medium) picks up ready tasks in dependency order, hands each to a Codex worker (gpt-5.6-sol low), verifies with real test runs (or `codex-computer-use` for UI rows, never a silent pass), keeps task logs, resumes after interruption, and commits per task. | auto — when `.tasks/okf/` artifacts exist |
