...
4. Find the baseline test command in checked-in repository instructions and build configuration. Stop with `status=needs_input` if it is missing or ambiguous. Run it on the starting commit. Stop with `status=blocked` and its failure summary if it fails. Record the exact command and result.
